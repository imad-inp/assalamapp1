...
public class KafilResource {
     ...
     *
     * @param kafil the kafil to create
     * @return the ResponseEntity with status 201 (Created) and with body the new kafil, or with status 400 (Bad Request) if the kafil has already an ID
    ...
    public ResponseEntity<Kafil> createKafil(@RequestBody Kafil kafil) throws URISyntaxException {
        ...
        if (kafil.getId() != null) {
            return ResponseEntity.badRequest().headers(HeaderUtil.createFailureAlert(ENTITY_NAME, "idexists", "A new kafil cannot already have an ID")).body(null);
        }
     ...
     *
     * @param kafil the kafil to update
    ...
    public ResponseEntity<List<Kafil>> getAllKafils(@ApiParam Pageable pageable) {
        ...
        Page<Kafil> page = kafilService.findAll(pageable);
  ...
  public ResponseEntity<List<Kafil>> searchKafils(@ApiParam Pageable pageable,
     ...
     *
     * @param id the id of the kafil to retrieve
     ...
     *
     * @param id the id of the kafil to delete
